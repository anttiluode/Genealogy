...
**AdaptiveObserverCache** has now crossed from synthetic retrieval gates into a real two-timescale Qwen experiment. The earlier DistilGPT2 chain established persistent observer control, then deliberately exposed its own slot/order confound: Gate 3 showed the first reusable coordinate tracked physical source position, while Gates 4–7 rebuilt durable provenance/current-address binding from explicit identity, aliases and finally behavioral history.

The Qwen branch produced a second useful correction. Attention geometry alone was not enough: the first steerable heads moved source attention without changing language, and one candidate head even had the wrong downstream causal sign. Selecting heads by downstream causal effect produced a real valve↔sensor language switch on frozen Qwen3-8B, but the held-out gate stayed mixed: the same frozen four-head plan moved complete A-vs-B sequence likelihood in the intended direction on **4/4** unrelated conflicts, while only **2/4** generated answers fully switched. An order-only attacker then isolated a large first-record prior in the unmodified model (about +10.88 versus -10.76 A-minus-B log-likelihood under AB/BA order) while the observer slope stayed almost unchanged (**7.91 vs 8.18; ratio 1.034**). The effect is therefore not explained by a simple first-slot coordinate, although the BA greedy sentence still failed to cross at m=+1 and the corrected verdict is likelihood-only identity control rather than a full behavioral pass.

The newest live-cache run closes a different gap. One Qwen `DynamicCache` now remains alive across turns instead of rerendering the transcript. On the first successful second turn it grew **131 → 149 → 160 tokens**, cache/history counts remained equal, source spans stayed fixed at **A=[45,62), B=[72,87)**, and the historical source-key integrity check remained true. An external B sensor receipt changed anchored observer trust to **m=-0.664** and moved selected-head attention strongly toward B (**0.2532 target vs 0.0362 other**), with no capped updates. The generated answer nevertheless stayed on A. So the earned claim is now precise: **persistent external observer state + one genuinely growing immutable-history cache works mechanically; robust language-level control across temporal distance does not yet.**

The next clean discriminator is a **distance × trust sweep on the same live cache**. Hold sources and frozen heads fixed, grow the cache to increasing distances, and remeasure complete A-vs-B sequence likelihood at matched trust values. That separates temporal-distance decay from the already-observed decoding threshold.

The strongest genealogy connections remain `OperatorTime` (resident history changes the operator available now), `ReadWrite` (the probe determines which hidden distinction is observable), `WhatToLookAt` (history changes which measurement is worth buying), `PredictiveHKT` (coordinates can move), `AuditedEpistemicCache` (old evidence needs a measurement receipt), and `AInstein` (confidence is not provenance). The Anthropic **J-space / Jacobian-lens** work remains an external conceptual comparison rather than an ancestry edge: J-space is an internal forward-pass workspace/control surface, while AdaptiveObserverCache is a persistent policy for how memory is interrogated across reads.
